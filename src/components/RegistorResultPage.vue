...
export default {
  ...
  methods: {
    ...
    salaryWon() {
      const { payday, salary, SalarySelect } = this.sec1Data;
      let { dailyIncomePrice } = this;
      console.log(payday, salary, SalarySelect);
      let result;
      const date = new Date().getDate();
      const month = new Date().getMonth();
      const lastDate = new Date(2020, month + 1, 0).getDate();
      const lastMonthDate = new Date(2020, month, 0).getDate();

      const countDate = date - payday;
      ...
      if (!this.minusDate) {
        ...
        console.log(payday - lastMonthDate - 1 + date, 'whatthefuck');
        console.log(payday, lastMonthDate, date);

        /* const lastDateCount = lastMonthDate - payday <= 0 ? this.minusDate : 0;
        result = Math.floor(
          parseFloat(dailyIncomePrice * (date + lastDateCount))
        ).toLocaleString();
        console.log(lastMonthDate - payday, 'whatthefuck'); */
        return result;
      }
